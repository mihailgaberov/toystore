...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create toystore-265f
  fly apps create toystore-265f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app toystore-265f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app toystore-265f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'toystore-265f-staging' does not match app name in config file 'toystore-265f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name toystore-265f-db
  fly postgres attach --app toystore-265f toystore-265f-db

  fly postgres create --name toystore-265f-staging-db
  fly postgres attach --app toystore-265f-staging toystore-265f-staging-db
  ```
